...
from Quantize5 import quantize_weights, to_hw_weight_list
from RTLPythonMirror5 import Net

#------ Pointer to MITBIH database (can be overridden by command-line arg) ------
...
DM_STEP_COARSE   = 80
DM_STEP_FINE     = 12
EVAL_THRESHOLDS  = [50, 50, 50, 50, 50]
EVAL_LEAK_RATE   = 12
EVAL_REFRACTORY  = 4


def delta_modulate(beat, step_size, min_val=0, max_val=2047):
    n = len(beat)
...
def beats_to_spike_tensor(beats, step_coarse=DM_STEP_COARSE, step_fine=DM_STEP_FINE):
    #Two DMs per beat -> 4 cheannels [up_c, down_c, up_f, down_f].
    T = len(beats[0])
    X = np.zeros((len(beats), T, 4), dtype=np.float32)
    for i, beat in enumerate(beats):
        b = np.asarray(beat)
        u_c, d_c = delta_modulate(b, step_coarse)
        X[i, :, 0] = u_c; X[i, :, 1] = d_c
        u_f, d_f = delta_modulate(b, step_fine)
        X[i, :, 2] = u_f; X[i, :, 3] = d_f
...
def inverse_frequency_weights(labels, n_classes=5, cap=50.0, q_index=4, q_weight=0.1):
    ...
    #minimize weight vector similarity
...
def evaluate_on_hardware(hw_weights, thresholds, beats, labels,
                         leak_rate, refractory_period, dm_step_coarse, dm_step_fine):
    net = Net(weights=hw_weights, thresholds=thresholds,
                  leak_rate=leak_rate, refractory_period=refractory_period,
                  dm_step_coarse=DM_STEP_COARSE, dm_step_fine=DM_STEP_FINE, n_neurons=5, n_inputs=4)
...
def main():
    ...
    print(f"\nDelta-modulating: {DM_STEP_COARSE}/{DM_STEP_FINE} -> 4 channels...")
    ...
    print(f"\nTraining DPE_Train (4->5)  epochs={EPOCHS} lr={LEARNING_RATE} beta={BETA} seed={SEED}")
    ...
    float_w = model.fc.weight.detach().cpu().numpy()   # (5,4)
    diagnose_weights(float_w)
    q = quantize_weights(float_w, weight_bits=6, verbose=True)
    hw_weights = to_hw_weight_list(q, n_neurons=5)
    print("Flat HW weight list (len 20):", hw_weights)

    if EVAL_THRESHOLDS is None:
        SOE = 4
        thresholds = [max(1, int(round(SOE * (max(abs(q[j,0]),abs(q[j,1]),abs(q[j,2]),abs(q[j,3])) or 1)))) for j in range(5)]
    ...
    print(f"Evaluating on Net: (leak={EVAL_LEAK_RATE}, refrac={EVAL_REFRACTORY}, dm step coarse ={DM_STEP_COARSE}, dm step fine ={DM_STEP_FINE})")
    confusion = evaluate_on_hardware(hw_weights, thresholds, test_beats, test_labels,
                                     EVAL_LEAK_RATE, EVAL_REFRACTORY, DM_STEP_COARSE, DM_STEP_FINE)
    print_confusion(confusion)
    np.savez("trained_weights_v5.npz", float_weights=float_w, quant_weights=q,
             hw_weights=hw_weights, thresholds=thresholds)
    print("\nSaved trained_weights_v5.npz (for the threshold sweep).")
