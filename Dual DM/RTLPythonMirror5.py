...
class Net:
    ...
    def __init__(self, weights, thresholds, leak_rate, refractory_period,
                 dm_step_coarse, dm_step_fine, n_neurons=5, n_inputs=4,
                ...
                self.dm_step_coarse = dm_step_coarse
                self.dm_step_fine = dm_step_fine
                self.dm_coarse = DeltaMod(data_w=data_w, min_val=min_val,max_val=max_val)
                self.dm_fine = DeltaMod(data_w=data_w, min_val=min_val,max_val=max_val)
    ...
    def step_pipeline(self, ecg, beat_clear=0):
        spikes_stale = [self.dm_coarse.up, self.dm_coarse.down, self.dm_fine.up, self.dm_fine.down]
        ...
        up_c, dn_c = self.dm_coarse.step(ecg, self.dm_step_coarse, self.enable)
        up_f, dn_f = self.dm_fine.step(ecg, self.dm_step_fine, self.enable)
        return {"fire": fire, "mp": [n.mp for n in self.dpe.neurons],
                "winner": winner, "valid": valid, "spikes": (up_c, dn_c, up_f, dn_f)}
